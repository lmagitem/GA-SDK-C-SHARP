...
namespace GameAnalyticsSDK.Net.State {
  public class GAState {
    ...
    public static string CurrentCustomDimension01 {
      get => Instance._currentCustomDimension01;
    ...
    public static string CurrentCustomDimension02 {
      get => Instance._currentCustomDimension02;
    ...
    public static string[] AvailableCustomDimensions01 {
      get => Instance._availableCustomDimensions01;
      set {
        // Validate
        if (!GAValidator.ValidateCustomDimensions(value)) return;
    ...
    public static string[] AvailableCustomDimensions02 {
      get => Instance._availableCustomDimensions02;
      set {
        // Validate
        if (!GAValidator.ValidateCustomDimensions(value)) return;
    ...
    public static string[] AvailableCustomDimensions03 {
      get => Instance._availableCustomDimensions03;
      set {
        // Validate
        if (!GAValidator.ValidateCustomDimensions(value)) return;
    ...
    public static string[] AvailableResourceCurrencies {
      get => Instance._availableResourceCurrencies;
      set {
        // Validate
        if (!GAValidator.ValidateResourceCurrencies(value)) return;
    ...
    public static string[] AvailableResourceItemTypes {
      get => Instance._availableResourceItemTypes;
      set {
        // Validate
        if (!GAValidator.ValidateResourceItemTypes(value)) return;
    ...
    public static bool UseManualSessionHandling {
      get => Instance._useManualSessionHandling;
    ...
    public static bool IsEventSubmissionEnabled {
      get => Instance._isEventSubmissionEnabled;
    ...
    private static JSONNode SdkConfig {
      get {
        if (Instance.sdkConfig.AsObject != null && Instance.sdkConfig.Count != 0) return Instance.sdkConfig;

        if (Instance.sdkConfigCached.AsObject != null && Instance.sdkConfigCached.Count != 0) return Instance.sdkConfigCached;

        return Instance.sdkConfigDefault;
      }
    }

    private readonly Dictionary<string, int> progressionTries = new Dictionary<string, int>();
    private readonly JSONNode sdkConfigDefault = new JSONObject();
    ...
    public static void SetCustomDimension01(string dimension) {
      ...
      if (GAStore.IsTableReady) GAStore.SetState(Dimension01Key, dimension);
    ...
    public static void SetCustomDimension02(string dimension) {
      ...
      if (GAStore.IsTableReady) GAStore.SetState(Dimension02Key, dimension);
    ...
    public static void SetCustomDimension03(string dimension) {
      ...
      if (GAStore.IsTableReady) GAStore.SetState(Dimension03Key, dimension);
...
#pragma warning disable 0162
    public static void IncrementProgressionTries(string progression) {
      var tries = GetProgressionTries(progression) + 1;
      Instance.progressionTries[progression] = tries;

      if (GAStore.InMemory) GALogger.D("Trying to IncrementProgressionTries with InMemory=true - cannot. Skipping.");
...
#pragma warning restore 0162
    ...
    public static int GetProgressionTries(string progression) {
      if (Instance.progressionTries.ContainsKey(progression)) return Instance.progressionTries[progression];
...
#pragma warning disable 0162
    public static void ClearProgressionTries(string progression) {
      var progressionTries = Instance.progressionTries;
      if (progressionTries.ContainsKey(progression)) progressionTries.Remove(progression);

      if (GAStore.InMemory) GALogger.D("Trying to ClearProgressionTries with InMemory=true - cannot. Skipping.");
...
#pragma warning restore 0162
    ...
    public static void SetKeys(string gameKey,
                               string gameSecret) {
      GameKey    = gameKey;
...
#endif
    {
      ...
      if (!GAStore.IsTableReady) return;
      ...
      if (IsEnabled()) GAEvents.EnsureEventQueueIsRunning();
    }
    ...
    public static void EndSessionAndStopQueue(bool endThread) {
      ...
      if (endThread) GAThreading.StopThread();
    }
    ...
    public static JSONObject GetEventAnnotations() {
      ...
      // Session number
      annotations.Add(SessionNumKey, new JSONNumber(IsAnonymous ? 1 : SessionNum));

      // type of connection the user is currently on (add if valid)
      var connection_type                                                                     = GADevice.ConnectionType;
      if (GAValidator.ValidateConnectionType(connection_type)) annotations["connection_type"] = connection_type;

      if (!string.IsNullOrEmpty(GADevice.GameEngineVersion)) annotations["engine_version"] = GADevice.GameEngineVersion;

      // remote configs
      if (Instance.remoteConfigs != null && Instance.remoteConfigs.Count > 0) annotations["configurations"] = Instance.remoteConfigs;

      // A/B testing
      if (!string.IsNullOrEmpty(Instance.AbId)) annotations["ab_id"]                = Instance.AbId;
      if (!string.IsNullOrEmpty(Instance.AbVariantId)) annotations["ab_variant_id"] = Instance.AbVariantId;
      ...
      if (!string.IsNullOrEmpty(Build)) annotations["build"] = Build;
    ...
    public static JSONObject GetInitAnnotations() {
      var initAnnotations = new JSONObject();

      if (string.IsNullOrEmpty(Identifier)) CacheIdentifier();

      GAStore.SetState("last_used_identifier", Identifier);

      initAnnotations["user_id"] = Identifier;
      ...
      initAnnotations["session_num"] = IsAnonymous ? 1 : SessionNum;
      initAnnotations["random_salt"] = SessionNum;
    ...
    public static long GetClientTsAdjusted() {
      ...
      var clientTsAdjustedInteger = clientTs + Instance.ClientServerTimeOffset;

      if (GAValidator.ValidateClientTs(clientTsAdjustedInteger)) return clientTsAdjustedInteger;
    ...
    private static void AddErrorEvent(string           baseMessage,
                                      EGAErrorSeverity severity,
                                      string           message) {
      if (!IsEventSubmissionEnabled) return;

      var now = DateTime.Now;
      if (!timestampMap.ContainsKey(baseMessage)) timestampMap.Add(baseMessage, now);
      if (!countMap.ContainsKey(baseMessage)) countMap.Add(baseMessage, 0);
      var diff        = now - timestampMap[baseMessage];
      var diffMinutes = (int)diff.TotalMinutes;
      if (diffMinutes >= 60) {
        countMap[baseMessage]     = 0;
        timestampMap[baseMessage] = now;
      }

      if (countMap[baseMessage] >= MaxErrorCount) return;
      ...
      if (fields != null) {
        ...
        foreach (var entry in fields)
          if (entry.Key == null || entry.Value == null) {
            ...
            var message     = string.Format(baseMessage, entry.Key, entry.Value);
            GALogger.W(message);
            ...
            if (GAUtilities.StringMatch(entry.Key, "^[a-zA-Z0-9_]{1," + MaxCustomFieldsKeyLength + "}$")) {
              if (entry.Value is string || entry.Value is char) {
                ...
                if (value.Length <= MaxCustomFieldsValueStringLength && value.Length > 0) {
                  ...
                  var baseMessage =
                    "ValidateAndCleanCustomFields: entry with key={0}, value={1} has been omitted because its value is an empty string or exceeds the max number of characters ({2})";
                  var message = string.Format(baseMessage, entry.Key, entry.Value, MaxCustomFieldsValueStringLength);
                  GALogger.W(message);
                  AddErrorEvent(baseMessage, EGAErrorSeverity.Warning, message);
                }
              ...
              var baseMessage =
                "ValidateAndCleanCustomFields: entry with key={0}, value={1} has been omitted because its key illegal characters, an empty or exceeds the max number of characters ({2})";
              var message = string.Format(baseMessage, entry.Key, entry.Value, MaxCustomFieldsKeyLength);
              GALogger.W(message);
              AddErrorEvent(baseMessage, EGAErrorSeverity.Warning, message);
            }
          } else {
            var baseMessage = "ValidateAndCleanCustomFields: entry with key={0}, value={1} has been omitted because it exceeds the max number of custom fields ({2})";
            var message     = string.Format(baseMessage, entry.Key, entry.Value, MaxCustomFieldsCount);
            GALogger.W(message);
            AddErrorEvent(baseMessage, EGAErrorSeverity.Warning, message);
          }
      }
    ...
    public static string GetRemoteConfigsStringValue(string key,
                                                     string defaultValue) {
    ...
    public static void AddRemoteConfigsListener(IRemoteConfigsListener listener) {
      if (!Instance.remoteConfigsListeners.Contains(listener)) Instance.remoteConfigsListeners.Add(listener);
    }


    public static void RemoveRemoteConfigsListener(IRemoteConfigsListener listener) {
      if (Instance.remoteConfigsListeners.Contains(listener)) Instance.remoteConfigsListeners.Remove(listener);
    }
...
#endif
      else if (!string.IsNullOrEmpty(Instance.DefaultUserId)) Identifier = Instance.DefaultUserId;
...
#pragma warning disable 0162
    private static void EnsurePersistedStates() {
      ...
      var state_dict       = new JSONObject();
      var results_ga_state = GAStore.ExecuteQuerySync("SELECT * FROM ga_state;");

      if (results_ga_state != null && results_ga_state.Count != 0)
        for (var i = 0; i < results_ga_state.Count; ++i) {
          var result = results_ga_state[i];
      ...
      if (!string.IsNullOrEmpty(CurrentCustomDimension01)) {
        ...
        if (!string.IsNullOrEmpty(CurrentCustomDimension01)) GALogger.D("Dimension01 found in cache: " + CurrentCustomDimension01);
      }
      ...
      if (!string.IsNullOrEmpty(CurrentCustomDimension02)) {
        ...
        if (!string.IsNullOrEmpty(CurrentCustomDimension02)) GALogger.D("Dimension02 found in cache: " + CurrentCustomDimension02);
      }
      ...
      if (!string.IsNullOrEmpty(CurrentCustomDimension03)) {
        ...
        if (!string.IsNullOrEmpty(CurrentCustomDimension03)) GALogger.D("Dimension03 found in cache: " + CurrentCustomDimension03);
      }

      // get cached init call values
      var sdkConfigCachedString = state_dict[SdkConfigCachedKey] != null ? state_dict[SdkConfigCachedKey].Value : "";
      if (!string.IsNullOrEmpty(sdkConfigCachedString)) {
        // decode JSON
        JSONNode sdkConfigCached = null;
        ...
        if (sdkConfigCached != null && sdkConfigCached.Count != 0) {
          var lastUsedIdentifier = state_dict["last_used_identifier"] != null ? state_dict["last_used_identifier"].Value : "";
          if (!string.IsNullOrEmpty(lastUsedIdentifier) && lastUsedIdentifier != Identifier) {
            GALogger.W("New identifier spotted compared to last one used, clearing cached configs hash!!");
            if (sdkConfigCached["configs_hash"] != null) sdkConfigCached["configs_hash"] = null;
          }
      ...
      {
        var currentSdkConfig = SdkConfig;
      ...
      var results_ga_progression = GAStore.ExecuteQuerySync("SELECT * FROM ga_progression;");

      if (results_ga_progression != null && results_ga_progression.Count != 0)
        for (var i = 0; i < results_ga_progression.Count; ++i) {
          var result                                                                                      = results_ga_progression[i];
          if (result != null && result.Count != 0) instance.progressionTries[result["progression"].Value] = result["tries"].AsInt;
        }
    }
#pragma warning restore 0162
...
#else
      var initResponse = GAHTTPApi.Instance.RequestInitReturningDict(Instance.ConfigsHash);
#endif
    ...
    public static void StartNewSession(EGAHTTPApiResponse initResponse,
                                       JSONObject         initResponseDict) {
      // init is ok
      if ((initResponse == EGAHTTPApiResponse.Ok || initResponse == EGAHTTPApiResponse.Created) && initResponseDict != null) {
        ...
        if (initResponseDict["server_ts"] != null) {
          var serverTs = initResponseDict["server_ts"].AsLong;
        ...
        if (initResponse != EGAHTTPApiResponse.Created) {
          ...
          if (currentSdkConfig["configs"]       != null && currentSdkConfig["configs"].IsArray) initResponseDict["configs"]              = currentSdkConfig["configs"].AsArray;
          if (currentSdkConfig["configs_hash"]  != null && currentSdkConfig["configs_hash"].IsString) initResponseDict["configs_hash"]   = currentSdkConfig["configs_hash"].Value;
          if (currentSdkConfig["ab_id"]         != null && currentSdkConfig["ab_id"].IsString) initResponseDict["ab_id"]                 = currentSdkConfig["ab_id"].Value;
          if (currentSdkConfig["ab_variant_id"] != null && currentSdkConfig["ab_variant_id"].IsString) initResponseDict["ab_variant_id"] = currentSdkConfig["ab_variant_id"].Value;
        }
        ...
        GALogger.D("initResponseDict: " + initResponseDict);
      ...
      var newSessionIdLowercase = newSessionId.ToLowerInvariant();
    ...
    private static void PopulateConfigurations(JSONNode sdkConfig) {
      lock (Instance.remoteConfigsLock) {
        var remoteConfigs = sdkConfig["configs"].AsArray;

        if (remoteConfigs != null) {
          Instance.remoteConfigs = new JSONObject();
          for (var i = 0; i < remoteConfigs.Count; ++i) {
            var configuration = remoteConfigs[i];
            ...
            if (configuration != null) {
              ...
              var start_ts = configuration["start_ts"].IsNumber ? configuration["start_ts"].AsLong : long.MinValue;
              var end_ts   = configuration["end_ts"].IsNumber ? configuration["end_ts"].AsLong : long.MaxValue;

              var client_ts_adjusted = GetClientTsAdjusted();
        ...
        foreach (var listener in Instance.remoteConfigsListeners) listener.OnRemoteConfigsUpdated();
      }
    }
