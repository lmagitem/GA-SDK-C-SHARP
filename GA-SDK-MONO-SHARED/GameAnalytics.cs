...
namespace GameAnalyticsSDK.Net {
  public static class GameAnalytics {
    ...
    internal static void MessageLogged(string               message,
                                       EGALoggerMessageType type) {
    ...
    public static void ConfigureSdkGameEngineVersion(string sdkGameEngineVersion) {
      ...
      GAThreading.PerformTaskOnGAThread("configureSdkGameEngineVersion", () => {
        if (IsSdkReady(true, false)) return;
    ...
    public static void ConfigureBuildPlatform(string input) {
      if (_endThread) return;

      GAThreading.PerformTaskOnGAThread("configureBuildPlatform", () => {
        if (IsSdkReady(true, false)) return;
        if (!GAValidator.ValidateShortString(input, false)) {
          GALogger.I("Validation fail - String: " + input);
          return;
        }

        GADevice.BuildPlatformOverride = input;
      });
    }


    public static void ConfigureOSVersion(string input) {
      if (_endThread) return;

      GAThreading.PerformTaskOnGAThread("configureOSVersion", () => {
        if (IsSdkReady(true, false)) return;
        if (!GAValidator.ValidateShortString(input, false)) {
          GALogger.I("Validation fail - String: " + input);
          return;
        ...
        GADevice.OsVersionOverride = input;
      });
    }


    public static void ConfigureDeviceModel(string input) {
      if (_endThread) return;

      GAThreading.PerformTaskOnGAThread("configureDeviceModel", () => {
        if (IsSdkReady(true, false)) return;
        if (!GAValidator.ValidateShortString(input, false)) {
          GALogger.I("Validation fail - String: " + input);
          return;
        ...
        GADevice.DeviceModelOverride = input;
      });
    }


    public static void ConfigureDeviceManufacturer(string input) {
      if (_endThread) return;

      GAThreading.PerformTaskOnGAThread("configureDeviceManufacturer", () => {
        if (IsSdkReady(true, false)) return;
        if (!GAValidator.ValidateShortString(input, false)) {
          GALogger.I("Validation fail - String: " + input);
          return;
        ...
        GADevice.DeviceManufacturerOverride = input;
      });
    }


    public static void ConfigureWritablePath(string input) {
      if (_endThread) return;

      GAThreading.PerformTaskOnGAThread("configureWritablePath", () => {
        if (IsSdkReady(true, false)) return;
        if (!GAValidator.ValidateShortString(input, false)) {
          GALogger.I("Validation fail - String: " + input);
          return;
        ...
        GADevice.WritablePathOverride = input;
      });
    }
    ...
    public static void Initialize(string gameKey,
      ...
      GAThreading.PerformTaskOnGAThread("initialize", () => {
        ...
        if (!GAValidator.ValidateKeys(gameKey, gameSecret)) {
          GALogger.W("SDK failed initialize. Game key or secret key is invalid. Can only contain characters A-z 0-9, gameKey is 32 length, gameSecret is 40 length. Failed keys - gameKey: " +
                     gameKey + ", secretKey: " + gameSecret);
          return;
        }

        GAState.SetKeys(gameKey, gameSecret);

        if (!GAStore.EnsureDatabase(false, gameKey)) GALogger.W("Could not ensure/validate local event database: " + GADevice.WritablePath);
    ...
    public static void AddBusinessEvent(string                      currency,
                                        int                         amount,
                                        string                      itemType,
                                        string                      itemId,
                                        string                      cartType,
      ...
      GAThreading.PerformTaskOnGAThread("addBusinessEvent", () => {
        if (!IsSdkReady(true, true, "Could not add business event")) return;
        // Send to events
    ...
    public static void AddResourceEvent(EGAResourceFlowType         flowType,
                                        string                      currency,
                                        float                       amount,
                                        string                      itemType,
                                        string                      itemId,
      ...
      GAThreading.PerformTaskOnGAThread("addResourceEvent", () => {
        if (!IsSdkReady(true, true, "Could not add resource event")) return;
    ...
    public static void AddProgressionEvent(EGAProgressionStatus        progressionStatus,
    ...
    public static void AddProgressionEvent(EGAProgressionStatus        progressionStatus,
      ...
      GAThreading.PerformTaskOnGAThread("addProgressionEvent", () => {
        if (!IsSdkReady(true, true, "Could not add progression event")) return;
    ...
    public static void AddDesignEvent(string                      eventId,
      ...
      GAThreading.PerformTaskOnGAThread("addDesignEvent", () => {
        if (!IsSdkReady(true, true, "Could not add design event")) return;
    ...
    public static void AddErrorEvent(EGAErrorSeverity            severity,
                                     string                      message,
                                     IDictionary<string, object> customFields = null,
                                     bool                        mergeFields  = false) {
      if (_endThread) return;
      ...
      GAThreading.PerformTaskOnGAThread("addErrorEvent", () => {
        if (!IsSdkReady(true, true, "Could not add error event")) return;
    ...
    public static void SetEnabledManualSessionHandling(bool flag) {
      ...
      GAThreading.PerformTaskOnGAThread("setEnabledManualSessionHandling", () => { GAState.SetManualSessionHandling(flag); });
    }
    ...
    public static void SetGlobalCustomEventFields(IDictionary<string, object> customFields) {
      ...
      GAThreading.PerformTaskOnGAThread("setGlobalCustomEventFields", () => { GAState.SetGlobalCustomEventFields(customFields); });
    }
...
#endif
        {
          if (!GAState.Initialized) return;

          if (GAState.IsEnabled() && GAState.SessionIsStarted()) GAState.EndSessionAndStopQueue(false);
    ...
    public static void OnResume() {
      ...
      GAThreading.PerformTaskOnGAThread("onResume", () => { GAState.ResumeSessionAndStartQueue(); });
    }
    ...
    private static bool IsSdkReady(bool   needsInitialized,
      ...
      if (!GAStore.IsTableReady) {
        if (warn) GALogger.W(message + "Datastore not initialized");
        return false;
      }

      // Is SDK initialized
      if (needsInitialized && !GAState.Initialized) {
        if (warn) GALogger.W(message + "SDK is not initialized");
        return false;
      }

      // Is SDK enabled
      if (needsInitialized && !GAState.IsEnabled()) {
        if (warn) GALogger.W(message + "SDK is disabled");
        return false;
      }

      // Is session started
      if (needsInitialized && !GAState.SessionIsStarted()) {
        if (warn) GALogger.W(message + "Session has not started yet");
        return false;
      }
